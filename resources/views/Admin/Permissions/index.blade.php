    ...
    <main class="d-flex flex-column w-100" style="margin-left: 15em;">
      <div class="p-4 p-lg-5 pb-2">
        <div class="d-flex flex-wrap align-items-end justify-content-between gap-3">
          <div>
            <h2 class="display-6 fw-black mb-1">Gestion des permissions</h2>
            <p class="mb-0 text-white-50">Administrez les accès et les rôles de votre boutique de bijoux berbères.</p>
          </div>
            <a href="/Admin/Permissions/AddPermission" class="btn fw-bold rounded-lg d-inline-flex align-items-center gap-1" style="background:var(--bb-primary);color:#fff;">
      ...
      <div class="p-4 p-lg-5 container-fluid">
        <div class="card-dark rounded mt-4">
          ...
          <div class="table-responsive">
            <table class="table table-permissions align-middle mb-0" style="background-color: #5A1A19">
              <thead>
                <tr>
                ...
                @if ($permissions->total() > 0)                  
                  @foreach ($permissions as $permission)
                    <tr>
                      ...
                      <td class="px-4 py-3 small">{{ $permission->description }}</td>
                      <td class="px-4 py-3 small">{{ $permission->created_at->format("Y-m-d") }}</td>
                      <td class="px-4 py-3 text-end">
                        <div class="d-inline-flex align-items-center gap-1">
                          <a href="/Admin/Permissions/EditPermission/{{ $permission->id }}" class="btn btn-sm text-white-50"><i class="bi bi-pencil text-success fw-bold fs-5"></i></a>
                          <form action="/Admin/Permissions/destroy/{{ $permission->id }}" method="post">
                            @csrf
                            @method("DELETE")
          ...
          <div class="p-3 d-flex align-items-center justify-content-between border-top border-opacity-25">
            <p class="mb-0 small text-white-50">Affichage de {{ $permissions->firstItem() }} à {{ $permissions->lastItem() }} sur {{ $permissions->total() }} utilisateurs</p>
            <div class="d-flex gap-2">{{ $permissions->links() }}</div>
          </div>
        </div>
    </main>